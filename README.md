...
- 📦🆓💰 [CrawlGraph](https://github.com/pucilpet/crawlgraph-mcp) — Backlink and referring-domain intelligence for any domain, built on Common Crawl's open hyperlink graph (passive, no active scanning). Four tools: inbound linking domains ranked by authority, link-gap between domains, outreach-target discovery, and dataset releases. `npx -y crawlgraph-mcp`. Free tier, no signup; paid API for full results.
