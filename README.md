...
- 🆓💰 [Edgrapi](https://edgrapi.com) — US government data as clean JSON on one key: SAM.gov contract opportunities, USAspending awards and spending, Grants.gov grants, House STOCK Act congressional trades, and SEC EDGAR filings (Form 4 insider trades, 8-K events, 13F holdings, 13D/G stakes, XBRL fundamentals, full-text search). 19 read-only tools, OAuth 2.1 with dynamic client registration or an API key, and every record links back to the official source. Congressional trades lag up to 45 days by law and report dollar ranges, House only. Free tier 100 credits/month, paid from $10/mo. Hosted MCP: https://api.edgrapi.com/mcp
