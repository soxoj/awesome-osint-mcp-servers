...
- 🆓💰 [Malwagon](https://malwagon.com) — Malware analysis sandbox over MCP. Look up a SHA-256, submit a hash, URL, command or package, poll the scan, read the derived report, and search past scans by indicator (IP, domain, URL, mutex, registry key, hash, JA3/JA4). Sample bytes and artifacts are never served over MCP, only derived data. Remote streamable HTTP at `https://malwagon.com/mcp`; `initialize` and `tools/list` work without a credential, tool calls take a bearer token from a free account.
